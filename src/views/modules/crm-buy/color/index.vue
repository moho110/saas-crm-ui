    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-buy:color:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-buy:color:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listColor, getColor, delColor, addColor, updateColor } from "@/api/crm-buy/color";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除采购计划明细表颜色编号为"' + ids + '"的数据项？').then(function() {
        return delColor(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-buy/color/export', {
        ...this.queryParams
