    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['crm-buy:kaipiaorecord:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['crm-buy:kaipiaorecord:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-buy:kaipiaorecord:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-buy:kaipiaorecord:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="kaipiaorecordList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="汇款ID" align="center" prop="huikuanid" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="是否汇款" prop="ifhuikuan">
          <el-input v-model="form.ifhuikuan" placeholder="请输入是否汇款" />
        </el-form-item>
...
<script>
import { listKaipiaorecord, getKaipiaorecord, delKaipiaorecord, addKaipiaorecord, updateKaipiaorecord } from "@/api/crm-buy/kaipiaorecord";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listKaipiaorecord(this.queryParams).then(response => {
        this.kaipiaorecordList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getKaipiaorecord(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateKaipiaorecord(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addKaipiaorecord(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除开票记录编号为"' + ids + '"的数据项？').then(function() {
        return delKaipiaorecord(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-buy/kaipiaorecord/export', {
        ...this.queryParams
      }, `kaipiaorecord_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
