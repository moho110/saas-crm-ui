  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['crm-hr:workexperience:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['crm-hr:workexperience:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-hr:workexperience:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-hr:workexperience:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="离职理由" prop="quitreason">
          <el-input v-model="form.quitreason" placeholder="请输入离职理由" />
        </el-form-item>
      </el-form>
...
<script>
import { listWorkexperience, getWorkexperience, delWorkexperience, addWorkexperience, updateWorkexperience } from "@/api/crm-hr/workexperience";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        quitreason: [
          { required: true, message: "离职理由不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listWorkexperience(this.queryParams).then(response => {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加工作人员工作经验";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getWorkexperience(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改工作人员工作经验";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除工作人员工作经验编号为"' + ids + '"的数据项？').then(function() {
        return delWorkexperience(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-hr/workexperience/export', {
        ...this.queryParams
      }, `workexperience_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
