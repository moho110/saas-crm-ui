<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="编号" prop="indepartment">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['crm-political:fixedassetin:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['crm-political:fixedassetin:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-political:fixedassetin:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-political:fixedassetin:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="fixedassetinList" @selection-change="handleSelectionChange">
...
<script>
import { listFixedassetin, getFixedassetin, delFixedassetin, addFixedassetin, updateFixedassetin } from "@/api/crm-political/fixedassetin";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFixedassetin(this.queryParams).then(response => {
        this.fixedassetinList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getFixedassetin(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateFixedassetin(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFixedassetin(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除固定资产入库编号为"' + ids + '"的数据项？').then(function() {
        return delFixedassetin(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-political/fixedassetin/export', {
        ...this.queryParams
      }, `fixedassetin_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
