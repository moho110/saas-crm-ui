  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['crm-political:fixedassetweixiu:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['crm-political:fixedassetweixiu:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-political:fixedassetweixiu:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-political:fixedassetweixiu:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="fixedassetweixiuList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="维修单位" align="center" prop="repairunit" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="维修单位" prop="repairunit">
          <el-input v-model="form.repairunit" placeholder="请输入维修单位" />
        </el-form-item>
        <el-form-item label="维修人" prop="repairman">
          <el-input v-model="form.repairman" placeholder="请输入维修单位" />
        </el-form-item>
        <el-form-item label="情况介绍" prop="thingintro">
          <el-input v-model="form.thingintro" placeholder="请输入情况介绍" />
        </el-form-item>
        <el-form-item label="批准人" prop="approvalman">
...
<script>
import { listFixedassetweixiu, getFixedassetweixiu, delFixedassetweixiu, addFixedassetweixiu, updateFixedassetweixiu } from "@/api/crm-political/fixedassetweixiu";

export default {
  name: "Fixedassetweixiu",
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        ...
        thingintro: [
          { required: true, message: "情况介绍不能为空", trigger: "blur" }
        ],
        approvalman: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFixedassetweixiu(this.queryParams).then(response => {
        this.fixedassetweixiuList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getFixedassetweixiu(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateFixedassetweixiu(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFixedassetweixiu(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除固定资产维修编号为"' + ids + '"的数据项？').then(function() {
        return delFixedassetweixiu(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-political/fixedassetweixiu/export', {
        ...this.queryParams
      }, `fixedassetweixiu_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
