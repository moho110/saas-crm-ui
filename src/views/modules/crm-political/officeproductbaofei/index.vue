<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="报废数量" prop="baofeiquantity">
        <el-input
          v-model="queryParams.baofeiquantity"
          placeholder="请输入报废数量"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['crm-political:officeproductbaofei:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['crm-political:officeproductbaofei:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-political:officeproductbaofei:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-political:officeproductbaofei:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="officeproductbaofeiList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="仓库入库" align="center" prop="inwarehouse" />
      <el-table-column label="报废数量" align="center" prop="baofeiquantity" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="仓库入库" prop="inwarehouse">
          <el-input v-model="form.inwarehouse" placeholder="请输入仓库入库" />
        </el-form-item>
        <el-form-item label="报废数量" prop="baofeiquantity">
          <el-input v-model="form.baofeiquantity" placeholder="请输入报废数量" />
        </el-form-item>
        <el-form-item label="报废申请人" prop="baofeiapplyman">
...
<script>
import { listOfficeproductbaofei, getOfficeproductbaofei, delOfficeproductbaofei, addOfficeproductbaofei, updateOfficeproductbaofei } from "@/api/crm-political/officeproductbaofei";

export default {
  name: "Officeproductbaofei",
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        ...
        inwarehouse: [
          { required: true, message: "仓库入库不能为空", trigger: "blur" }
        ],
        baofeiquantity: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOfficeproductbaofei(this.queryParams).then(response => {
        this.officeproductbaofeiList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getOfficeproductbaofei(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOfficeproductbaofei(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOfficeproductbaofei(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除办公用品报废编号为"' + ids + '"的数据项？').then(function() {
        return delOfficeproductbaofei(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-political/officeproductbaofei/export', {
        ...this.queryParams
      }, `officeproductbaofei_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
