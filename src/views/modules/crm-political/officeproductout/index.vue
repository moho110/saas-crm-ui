    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['crm-political:officeproductout:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['crm-political:officeproductout:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-political:officeproductout:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-political:officeproductout:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="officeproductoutList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="出库仓库" align="center" prop="outwarehouse" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="出库仓库" prop="outwarehouse">
          <el-input v-model="form.outwarehouse" placeholder="请输入出库仓库" />
        </el-form-item>
        ...
        <el-form-item label="审核状态" prop="outstatus">
          <el-radio v-model="form.outstatus" label="1">是</el-radio>
          <el-radio v-model="form.outstatus" label="0">否</el-radio>
        </el-form-item>
        ...
        <el-form-item label="归还限制" prop="returnlimited">
          <el-input v-model="form.returnlimited" placeholder="请输入归还限制" />
        </el-form-item>
        ...
        <el-form-item label="归还收接人" prop="returnreiceiver">
          <el-input v-model="form.returnreiceiver" placeholder="请输入归还收接人" />
        </el-form-item>
        <el-form-item label="实际归还日期" prop="realreturndate">
          <el-date-picker clearable
            v-model="form.realreturndate"
            type="date"
            value-format="yyyy-MM-dd"
...
<script>
import { listOfficeproductout, getOfficeproductout, delOfficeproductout, addOfficeproductout, updateOfficeproductout } from "@/api/crm-political/officeproductout";

export default {
  name: "Officeproductout",
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        ...
        returnlimited: [
          { required: true, message: "归还限制不能为空", trigger: "blur" }
        ],
        isreturn: [
          { required: true, message: "是否归还不能为空", trigger: "blur" }
        ],
        returnreiceiver: [
          { required: true, message: "归还接收人不能为空", trigger: "blur" }
        ],
        realreturndate: [
          { required: true, message: "实际归还日期不能为空", trigger: "blur" }
        ],
        examinetime: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOfficeproductout(this.queryParams).then(response => {
        this.officeproductoutList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getOfficeproductout(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOfficeproductout(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOfficeproductout(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除办公用品出库编号为"' + ids + '"的数据项？').then(function() {
        return delOfficeproductout(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-political/officeproductout/export', {
        ...this.queryParams
      }, `officeproductout_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
