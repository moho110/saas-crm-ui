    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['crm-customer:customerdetail:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['crm-customer:customerdetail:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-customer:customerdetail:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-customer:customerdetail:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="customerdetailList" @selection-change="handleSelectionChange">
...
<script>
import { listCustomerdetail, getCustomerdetail, delCustomerdetail, addCustomerdetail, updateCustomerdetail } from "@/api/crm-customer/customerdetail";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCustomerdetail(this.queryParams).then(response => {
        this.customerdetailList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCustomerdetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCustomerdetail(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除产品明细表编号为"' + ids + '"的数据项？').then(function() {
        return delCustomerdetail(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-customer/customerdetail/export', {
        ...this.queryParams
      }, `customerdetail_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
