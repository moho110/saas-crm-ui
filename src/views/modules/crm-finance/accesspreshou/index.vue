  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['crm-finance:accesspreshou:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['crm-finance:accesspreshou:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-finance:accesspreshou:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-finance:accesspreshou:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="accesspreshouList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="ID" align="center" prop="id" />
      <el-table-column label="客户ID" align="center" prop="customerid" />
      <el-table-column label="联系人" align="center" prop="linkman" />
      <el-table-column label="客户ID" align="center" prop="accountid" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="部门清单ID" prop="billdeptid">
          <el-input v-model="form.billdeptid" placeholder="请输入部门清单ID" />
        </el-form-item>
      </el-form>
...
<script>
import { listAccesspreshou, getAccesspreshou, delAccesspreshou, addAccesspreshou, updateAccesspreshou } from "@/api/crm-finance/accesspreshou";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listAccesspreshou(this.queryParams).then(response => {
        this.accesspreshouList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getAccesspreshou(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateAccesspreshou(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAccesspreshou(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除预收款记录编号为"' + ids + '"的数据项？').then(function() {
        return delAccesspreshou(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-finance/accesspreshou/export', {
        ...this.queryParams
      }, `accesspreshou_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
