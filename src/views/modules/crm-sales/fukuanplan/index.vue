    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['crm-sales:fukuanplan:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['crm-sales:fukuanplan:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-sales:fukuanplan:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-sales:fukuanplan:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="fukuanplanList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="是否支付" align="center" prop="ifpay" />
...
<script>
import { listFukuanplan, getFukuanplan, delFukuanplan, addFukuanplan, updateFukuanplan } from "@/api/crm-sales/fukuanplan";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFukuanplan(this.queryParams).then(response => {
        this.fukuanplanList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getFukuanplan(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateFukuanplan(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFukuanplan(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除付款计划编号为"' + ids + '"的数据项？').then(function() {
        return delFukuanplan(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-sales/fukuanplan/export', {
        ...this.queryParams
      }, `fukuanplan_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
