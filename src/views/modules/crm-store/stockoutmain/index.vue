    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['crm-store:stockoutmain:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['crm-store:stockoutmain:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-store:stockoutmain:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-store:stockoutmain:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="stockoutmainList" @selection-change="handleSelectionChange">
...
<script>
import { listStockoutmain, getStockoutmain, delStockoutmain, addStockoutmain, updateStockoutmain } from "@/api/crm-store/stockoutmain";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listStockoutmain(this.queryParams).then(response => {
        this.stockoutmainList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getStockoutmain(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateStockoutmain(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addStockoutmain(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除仓库出库编号为"' + ids + '"的数据项？').then(function() {
        return delStockoutmain(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-store/stockoutmain/export', {
        ...this.queryParams
      }, `stockoutmain_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
