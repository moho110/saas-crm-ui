    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['crm-store:changmaindetail:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['crm-store:changmaindetail:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['crm-store:changmaindetail:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm-store:changmaindetail:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="changmaindetailList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="折扣" align="center" prop="zhekou" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="折扣" prop="zhekou">
          <el-input v-model="form.zhekou" placeholder="请输入折扣" />
        </el-form-item>
        ...
        <el-form-item label="操作类型" prop="opertype">
          <el-radio v-model="form.oldprodid" label="1">特殊</el-radio>
          <el-radio v-model="form.oldprodid" label="0">一般</el-radio>
        </el-form-item>
      </el-form>
...
<script>
import { listChangmaindetail, getChangmaindetail, delChangmaindetail, addChangmaindetail, updateChangmaindetail } from "@/api/crm-store/changmaindetail";

export default {
  name: "Changmaindetail",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listChangmaindetail(this.queryParams).then(response => {
        this.changmaindetailList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加仓库管理调拔明细";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getChangmaindetail(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改仓库管理调拔明细";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateChangmaindetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addChangmaindetail(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除仓库管理调拔明细编号为"' + ids + '"的数据项？').then(function() {
        return delChangmaindetail(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm-store/changmaindetail/export', {
        ...this.queryParams
      }, `changmaindetail_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
